...
\title{Quality control metrics for targeted RNA-Seq: methods afforded by a compositional framework.}
...
We propose an alternative visualization for detecting simultaneously detecting both violations of compositional invariance. We use the multivariate Aitchison distance (\ref{aitchdist2}) between all pairs of samples. If CI is violated we expect pairs samples with similar total aligned reads will have smaller scalar distances. If the variance depends on the total aligned reads we expect the scalar distance between sample pairs to decrease with increasing read depth.\\

We demonstrate this visualization on ...

%Use euclidean distance matrix with samples ordered by read depth
...
<<CompositionalInvariance, echo = FALSE>>=
###############################################################################
###############################################################################
...
miRNA.heat <- ggplot(midl, aes(x = col.sample, y = row.sample)) + 
  ...
  plotTheme(axis.text.x = element_text(angle = 90, vjust = .5))
...
io.heat <- ggplot(iodl, aes(x = col.sample, y = row.sample)) + 
  ...
  geom_tile(data = iodltots, 
            ...
            fill = "lightgrey", 
            lwd = 3) +
  scale_fill_gradient("Distance", na.value = "lightgrey") +
  scale_color_gradientn("log Total Reads", colours = c("white", "orange", "red")) +
  xlab("Sample") + 
  ylab("Sample") +
  plotTheme(axis.text.x = element_text(angle = 90, vjust = .5))

ggsave("./Figures/Distance_Heatmap_IO_allSamples.png", io.heat, height = 6, width = 7)
@
