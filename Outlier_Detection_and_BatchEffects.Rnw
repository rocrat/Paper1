...
Up to this point we have referred to the total reads available per sequencing run, $\mathbb{T}$.  However, it is more typical to work with the aligned reads in practice.  The total aligned reads, $T$, is always a fraction of the total reads available for a sequencing run, $\mathbb{T}$.  The fraction of the total reads aligned can be a affected by multiple factors, including the choice of alignment algorithm, which we do not address here.  We assume that $T$ imposes the same constraints on the data as oultined above for $\mathbb{T}$ and will refer exclusively to $T$ hereafter.\\  
  ...
  Aitchison~\cite{Aitchison1986} defines this property as \emph{Compositional Invariance}.  
...
It is well documented that the quality of RNA-Seq depends on the read depth of the sequencing run with higher read-depths associated with higher quality data~\cite{CITE}.  This is clearly a violation of compositional invariance (CI).  We derive a test for compositional invariance in high-dimensional RNA-Seq assays to ensure that the differences in expression among samples is independent of the differences in total counts among samples.\\
...
# 
# #Normalize together
# dfw <- plyr::join_all(dflist, by = "X0")
# rownames(dfw) <- dfw[, 1]
# names(dfw) <- dfw[3, ]
...
#log data
pcaDatl.mi <- apply(pcaDat.mi.orig, c(1,2), function(x) log2(x + 1))
...
#log perturbed data
pcaDatl.mi.p <- apply(pcaDat.mi, c(1,2), function(x) log2(x + 1))

pcaDatl.mi.brain.p <- pcaDatl.mi.p[, which(grepl("Brain", colnames(pcaDatl.mi.p)))]
pcplotdatl.mi.brain <- plotPCA(pcaDatl.mi.brain.p, annodf.mi[which(grepl("Brain", annodf.mi$SampType)), ], scale = FALSE)
ggpcal.mi.l.brain.perturbed <- ggplot(pcplotdatl.mi.brain[[1]], aes(x = PC1, y = PC2, color = Run)) + 
    geom_point( size = 3) +
    coord_equal() + ylim(c(-30, 100)) + xlim(c(-50, 160)) +
  ggtitle(paste0("Cum. proportion of var. = ", round(pcplotdatl.mi.brain[[2]], 2)))
ggsave("./Figures/miRNA_Repro_PCA_log(raw)_BrainOnly_perturbed.png", ggpcal.mi.l.brain, width = 9, height = 8 )
