...
ggplot(mialr.plasma.test$betas, aes(x = as.numeric(as.character(est)))) + 
  geom_density()
...
ggplot(mialr.brain.test$betas, aes(x = as.numeric(as.character(est)))) + 
  geom_density()
...
tq <- quantile(midltots.plas$total, probs = .75)
tqsamps <- midltots.plas[which(midltots.plas$total > tq), "row.sample"]

cent <- apply(miclr[, tqsamps], 1, mean)

distances <- apply(t(miclr), 1, function(x) dist(rbind(x, cent)))

dist.plas <- distances[which(grepl("Plasma", names(distances)))]

##Create data frame:
plasddf <- data.frame(SampleID = names(dist.plas),
                      distance = dist.plas,
                      total = CIanno[names(dist.plas), ]$total)

##Order by total reads
plasddf$SampleID <- factor(plasddf$SampleID, 
                           levels = miorder[which(grepl("Plasma", miorder))])


ggplot(plasddf, aes(x = SampleID, y = distance)) + 
  geom_point(aes(size = log(total), color = log(total))) +
  plotTheme(axis.text.x = element_blank()) +
  ylab("Distance between sample and \ncenter of high reads samples" ) +
  xlab("Samples ordered by total aligned reads") +
  scale_size_continuous("Log(total", breaks = round(as.numeric(breaks), 2)) + 
  scale_color_continuous("Log(total", breaks = round(as.numeric(breaks), 2))
