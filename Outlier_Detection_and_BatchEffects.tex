...
Aitchison (\cite{Aitchison1986, Aitchison1992}) suggests using the sum of squares if all log-ratio differnces.  Billheimer, Guttorp, and Fagan (2001) use the geometry of compositions to define a norm which, along with the perturbation operator defined by Aitchison (\cite{Aitchison1986}), allow the interpretation of differences in compositions (\cite{Billherimer2001}). Briefly, denote the elementwise multiplication of two positive k-vectors \textbf{u} and \textbf{v} by  

$$\mathbf{u}\cdot \mathbf{v} \equiv \left( u_1v_1, u2_v2, ..., u_kv_k\right)^\prime.$$

Further define the perturbation operator for composition \textbf{x} and perturbation $\mathbf{\alpha} \in S^{d}$ as

$$z = \mathbf{x}\oplus \alpha = C\left(\mathbf{x}\alpha \right)$$
